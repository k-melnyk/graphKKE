...
import graph_simulation as graph_simulation
import generate_SDE as generate_SDE
...
parser.add_argument('--n_wells', type=int,
                    default=5)
parser.add_argument('--in_state', type=int,
                    default=0.9)
...
if __name__ == '__main__':
    lm = generate_SDE.LemonSlice2D([0.9, 0.9], args.n_graphs, 2)

    x = rand(args.n_graphs, np.asarray([[-0.5, 0.5], [-0.5, 0.5]]), np.asarray([40, 40]))
    ...
    sim_graph = graph_simulation.LemonGraph(args.radius, args.n_graphs, args.n_nodes,
                                            graph_states)
    graphs, images, node_points = sim_graph.create_adj_matrix(sde_traj, args.in_state, args.if_plot)
